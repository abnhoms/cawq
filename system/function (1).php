...
function mainMuted(){
	global $data;
	if(isMuted($data) || isMainMuted($data) || !inChat($data) || isRoomMuted($data) || !canMain() || isWarned($data) || !boomAllow($room['room_talk'])){
		return true;
	}
}
...
function joinRoomMessage($room){
	global $lang, $data;
	if(useLogs(1) && moveMessage($data) && isVisible($data) && !isOwner($data)){
		$content = str_replace(
			'@rank@',
			rankTitle($data['user_rank']),
			'انضم للغرفة (# @rank@ #) '
		);
		userPostChat($content, array("type" => "join__log", "room" => $room));
	}
}
...
function createLog($log, $quote = []){
    ...
    return [
        ...
        'user_auth' => (int) $log['user_auth'], // تم التأكيد هنا
...
function getChatLog($id){
	...
	$get_log = $mysqli->query("
		...
		boom_users.user_cover, boom_users.country, boom_users.user_bot, boom_users.ashare, boom_users.sshare, boom_users.lshare,boom_users.user_auth
		FROM boom_chat
...
function checkRateLimit(){
    ...
    $use_rate   = isset($setting['use_rate'])   ? (int)$setting['use_rate']   : 0;
    $rate_limit = isset($setting['rate_limit']) ? (int)$setting['rate_limit'] : 10;
    ...
    if (!isset($_SESSION[BOOM_PREFIX .'ftime']) || $_SESSION[BOOM_PREFIX . 'ftime'] < (time() - 20)) {
        ...
        return false;
    } elseif ((isset($_SESSION[BOOM_PREFIX . 'fcount']) ? $_SESSION[BOOM_PREFIX . 'fcount'] : 0) >= $rate_limit) {
        $_SESSION[BOOM_PREFIX . 'fignore'] = time() + 300;
        return true;
    }

    $_SESSION[BOOM_PREFIX . 'fcount'] = (isset($_SESSION[BOOM_PREFIX . 'fcount']) ? $_SESSION[BOOM_PREFIX . 'fcount'] : 0) + 1;
    return false;
}
function handleUserTalk(): void
{
    global $data, $mysqli;
    if (isMember($data)) {
        $mysqli->query("UPDATE boom_users SET umost_talk = umost_talk + 1  WHERE user_id = '{$data['user_id']}'");
    }
}
function handleRoomTalk(): void
{
    global $data, $mysqli;
    if (isMember($data)) {
        $mysqli->query("UPDATE boom_rooms SET talk_count = talk_count + 1  WHERE room_id = '{$data['user_roomid']}'");
    }
}
function checkTopTalkadmin(){
	global $mysqli;
    $checkTop3 = $mysqli->query("SELECT user_id FROM boom_users WHERE umost_talk > 0 AND user_rank < 100 AND user_rank >= 97 AND user_bot = 0 ORDER BY talk_count DESC LIMIT 3");
	if ($checkTop3->num_rows == 3) {
		return true;
	}
}
function checkTopTalk(){
	global $mysqli;
	$checkTop3 = $mysqli->query("SELECT * FROM boom_users WHERE umost_talk > 0 AND user_rank < 100 AND user_rank > 0 AND user_bot = 0 ORDER BY talk_count DESC LIMIT 3");
	if ($checkTop3->num_rows == 3) {
		return true;
	}
}
function checkTopTalkRooms(){
	global $mysqli;
	$checkTop3 = $mysqli->query("SELECT * FROM boom_rooms WHERE talk_count > 0 ORDER BY mcredit DESC LIMIT 3");
	if ($checkTop3->num_rows == 3) {
		return true;
	}
}
# fetch user location/device information
function reloadDeviceInformation($user_id)
{
    global $data, $redis, $mysqli, $x;
    // fetch permission
    if (!isOwner($data)) {
        return 0;
    }

    // Fetch user IP and details
    $userDetails = userDetails($user_id);
	$user_ip = $userDetails['user_ip'];
    $uadev = $userDetails['device_ua'];

    // Detect user location details using GeoIP
    $reader = new GeoIp2\Database\Reader($x['geo_file_city']);
    try {
        $record = $reader->city($user_ip);
    } catch (\Exception $e) {
        // Handle the case where the location can't be determined
        $record = null;
    }

    // Detect device details using DeviceDetector
    $detect = new DeviceDetector($uadev);
    $detect->parse();

    $userInfo = $detect->getClient();
    $osInfo = $detect->getOs();
    $ubrowser = $userInfo['name'] ?? 'Unknown';
    $ubrowser_ver = $userInfo['engine_version'] ?? 'Unknown';
    $device = $detect->getDeviceName() ?? 'Unknown Device';
    $brand = $detect->getBrandName() ?? '';
    $model = $detect->getModel() ?? '';
    $osInfoName = $osInfo['name'] ?? 'Unknown OS';
    $osVersion = $osInfo['version'] ?? 'Unknown Version';
    $platform = $osInfo['platform'] ?? '';

    // Determine if the device is mobile or not 
    $isMobile = ($osInfoName !== 'Windows') ? 1 : 0;
    $wver = $isMobile ? $osVersion : $osVersion . ' / ' . $platform;

    // Fetch users with the same device UA
    $getsame = $mysqli->query("SELECT user_name, user_id FROM boom_users WHERE device_ua = '{$uadev}' AND user_id != '{$user_id}' AND user_bot = 0 ORDER BY user_id DESC LIMIT 50");
    $sameUsers = [];
    if ($getsame && $getsame->num_rows > 0) {
        while ($usame = $getsame->fetch_assoc()) {
            $sameUsers[] = '<a onclick="getProfile(' . $usame['user_id'] . ');">' . htmlspecialchars($usame['user_name'], ENT_QUOTES, 'UTF-8') . '</a>';
        }
    }
    $sames = listThisArray($sameUsers);
    ...
    $locationData = [
        ...
        "postal" => $record->country->isoCode ?? 'Unknown',
        "latitude" => $record->location->latitude ?? 'Unknown',
        "longitude" => $record->location->longitude ?? 'Unknown',
        "timezone" => $record->traits->network ?? 'Unknown',
    ];

    // Prepare device data
    $deviceData = [
        "ip_addr" => $user_ip,
        "jdevice_info" => $device . ($brand ? ' / ' . $brand : '') . ($model ? ' / ' . $model : ''),
        "jdevice_mobile" => $isMobile,
        "jdevice_ua" => $uadev,
        "jdevice_ver" => $osInfoName . ' / ' . $wver,
        "jandroid_ver" => $osVersion,
        "juser_browser" => $ubrowser,
        "jbrowser_ver" => $ubrowser_ver,
        "jdevice_same" => $sames,
    ];

    // Merge user details with location and device data
    $merge = array_merge($userDetails, $locationData, $deviceData);

    // Render the user info template
    $uinfo = boomTemplate('xelement/x_device_info', $merge);
    return $uinfo;
}

# check user banned device/country/browser
function checkBanDevice($ua)
{
    global $mysqli, $x;
    ...
    $device = $detect->getDeviceName() ?: 'Unknown Device';
    $brand = $detect->getBrandName() ? ' / ' . $detect->getBrandName() : '';
    $model = $detect->getModel() ? ' / ' . $detect->getModel() : '';
    $osInfo = $detect->getOs();
    $osInfoName = $osInfo['name'] ?: 'Unknown OS';
    $osVersion = $osInfo['version'] ?: 'Unknown Version';
    $platform = $osInfo['platform'] ?: '';
    ...
    $isMobile = $osInfoName !== 'Windows' ? 1 : 0;
    $version = $isMobile ? $osVersion : $osVersion . ' / ' . $platform;

    $deviceCode = sha1(str_rot13($device . $brand . $model . $osInfoName . $version . $isMobile . $ua));
    $fullDevice = $device . $brand . $model;

    $stmt = $mysqli->prepare("SELECT 1 FROM boom_device WHERE device = ? AND dversion = ? AND status = '1'");
    $stmt->bind_param('ss', $fullDevice, $version);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0 && !boomAllow($x['bypass_full_ban'])) {
        return true;
    }

    return false;
}
function checkBanBrowser()
{
    global $mysqli, $x;
    ...
    $userInfo = $detect->getClient();
    $ubrowser = $userInfo['name'] ?: 'Unknown Browser';
    $ubrowser_ver = $userInfo['engine_version'] ?: 'Unknown Version';

    $stmt = $mysqli->prepare("SELECT 1 FROM boom_browser WHERE browser = ? AND bversion = ? AND status = '1'");
    $stmt->bind_param('ss', $ubrowser, $ubrowser_ver);
    $stmt->execute();
    $stmt->store_result();
...
function checkBanCountry()
{
    global $mysqli, $data, $x;
    ...
    try {
        $reader = new GeoIp2\Database\Reader($x['geo_file_country']);
        $record = $reader->country($data['user_ip']);
        $country_name = $record->country->name ?: 'Unknown Country';

        $stmt = $mysqli->prepare("SELECT 1 FROM boom_country WHERE country = ? AND status = '1'");
...
function isBrowserBan($b, $v)
{
    global $mysqli;
    $stmt = $mysqli->prepare("SELECT 1 FROM boom_browser WHERE browser = ? AND bversion = ? AND status = '1'");
    $stmt->bind_param('ss', $b, $v);
    $stmt->execute();
    $stmt->store_result();
...
function isDeviceBan($d, $v)
{
    global $mysqli;
    $stmt = $mysqli->prepare("SELECT 1 FROM boom_device WHERE device = ? AND dversion = ?");
    $stmt->bind_param('ss', $d, $v);
    $stmt->execute();
    $stmt->store_result();
...
function isFingerprintBan($f, $u)
{
    global $mysqli;
    $stmt = $mysqli->prepare("SELECT 1 FROM boom_user_fp WHERE fp = ? AND user_id = ?");
    $stmt->bind_param('si', $f, $u);
    $stmt->execute();
    $stmt->store_result();
...
function isFpBlocked($f)
{
    global $mysqli;
    $stmt = $mysqli->prepare("SELECT 1 FROM boom_user_fp WHERE fp = ?");
    $stmt->bind_param('s', $f);
    $stmt->execute();
    $stmt->store_result();
...
function fetchDeviceBanned()
{
    global $mysqli, $setting, $lang;
    $pg = '';
    $getCount = $mysqli->query("SELECT * FROM boom_device");
    if ($getCount->num_rows > 0) {
        while ($user = $getCount->fetch_assoc()) {
            $pg .= boomTemplate('element/x_fetch_device', $user);
        }
    } else {
...
function fetchBrowserBanned()
{
    global $mysqli, $setting, $lang;
    $pg = '';
    $getCount = $mysqli->query("SELECT * FROM boom_browser");
    if ($getCount->num_rows > 0) {
        while ($user = $getCount->fetch_assoc()) {
            $pg .= boomTemplate('element/x_fetch_browser', $user);
        }
    } else {
...
function fetchCountryBanned()
{
    global $mysqli, $setting, $lang;
    $pg = '';
    $getCount = $mysqli->query("SELECT * FROM boom_country");
    if ($getCount->num_rows > 0) {
        while ($user = $getCount->fetch_assoc()) {
            $pg .= boomTemplate('element/x_fetch_country', $user);
        }
    } else {
        $pg = emptyZone('No Country Found');
    }
    return $pg;
}
function fetchFingerprintBanned()
{
    global $mysqli, $setting, $lang;
    $pg = '';
    $getCount = $mysqli->query("SELECT * FROM boom_user_fp");
    if ($getCount->num_rows > 0) {
        while ($user = $getCount->fetch_assoc()) {
            $pg .= boomTemplate('element/x_fetch_fingerprint', $user);
        }
    } else {
        $pg = emptyZone('No xBanned Found');
    }
    return $pg;
}
?>